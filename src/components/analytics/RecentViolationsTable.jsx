...
const actionTint = {
  Open: 'text-[var(--color-accent-light)]',
  'In Review': 'text-[var(--color-warn)]',
  Closed: 'text-[var(--color-text-faint)]',
}

export default function RecentViolationsTable({ rows }) {
  return (
    <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-5">
      <div className="flex items-center justify-between">
        <p className="font-display text-sm font-bold">Recent Violations</p>
        <a href="#" className="text-xs font-medium text-[var(--color-accent-light)] hover:underline">
          View all violations
        </a>
      </div>
      ...
      <div className="mt-4 overflow-x-auto">
        <table className="w-full min-w-[720px] border-collapse text-left text-sm">
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.id} className="border-b border-[var(--color-border)]/60 text-[13px] last:border-0">
                ...
                <td className={`py-2.5 text-xs font-medium ${actionTint[r.action]}`}>{r.action}</td>
              </tr>
            ))}
          </tbody>
        </table>
